...
public:
    ...
    rclcpp::Node::SharedPtr n; // ros::NodeHandle n;
    tf2::Quaternion q_lidar_to_cam; // tf::Quaternion q_lidar_to_cam;
    Eigen::Quaterniond q_lidar_to_cam_eigen;

    rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pub_latest_odometry; // ros::Publisher pub_latest_odometry; 

    odometryRegister(std::shared_ptr<rclcpp::Node> n_in):
    n(n_in)
    {
    ...
    vector<float> getOdometry(deque<nav_msgs::msg::Odometry>& odomQueue, double img_time)
    {
        ...
        q_odom_lidar.setRPY(odomCur.pose.pose.orientation.x,odomCur.pose.pose.orientation.y,odomCur.pose.pose.orientation.z);
        ...
        tf2::Quaternion q;
        q.setRPY(0, 0, M_PI);
        q_odom_cam = q * (q_odom_lidar * q_lidar_to_cam);
        // tf2::fromMsg(q_odom_cam, odomCur.pose.pose.orientation);
